...
Matrix operator+(const Matrix & lhs, const Matrix & rhs){ //this adds two matrix and returns a matrix
	Matrix copy1 = Matrix(lhs);
	int row = copy1.getRowNumber();
	int col = copy1.getColumnNumber();
	for(int i = 0; i < row; i++){
		for(int j = 0; j < col; j++){
			int value = lhs.getElementAt(i,j) + rhs.getElementAt(i,j);
			copy1.setElementAt(i,j,value);
		}
...
Matrix operator-(const Matrix & lhs, const Matrix & rhs){ //this function subtracts two matrix
	Matrix copy1 = Matrix(lhs);
	int row = copy1.getRowNumber();
	int col = copy1.getColumnNumber();
	for(int i = 0; i < row; i++){
...
Matrix operator !(const Matrix & rhs){ //this matrix takes a transpose of the matrix
	if(rhs.getColumnNumber() == 0 && rhs.getRowNumber() == 0)
		return rhs;
	Matrix copy1(rhs.getColumnNumber(),rhs.getRowNumber(),0);
	for(int i= 0; i < copy1.getRowNumber();i++){
		for(int j = 0 ;j < copy1.getColumnNumber();j++){
			int temp = rhs.getElementAt(j,i); 
				copy1.setElementAt(i,j,temp);
		}
	}
